    ...
    for root_dir, _, files in os.walk(output_path):
        for f in files:
            ...
            if png_var.get() and f.lower().endswith(".png"):
                subprocess.run([pingo, f"-s{png_level.get()}", path],
                               stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

            if jpg_var.get() and f.lower().endswith((".jpg", ".jpeg")):
                subprocess.run([pingo, f"-quality={jpg_quality.get()}", path],
                               stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
# ---------- RUN ----------
ctk.CTkButton(main, text="Compress Images",
              height=45,
              corner_radius=10,
              command=compress).pack(pady=20, padx=10, fill="x")
